  ...
  try {
    ...
    for (const [i, width] of [390, 1440].entries()) {
      ...
      page.on("websocket", (ws) =>
        ws.on("socketerror", (e) => console.error("WS_ERROR", String(e))),
      );
      ...
      if (process.env.EFFECT_TEST)
        await page.getByLabel("不洗牌", { exact: true }).check();
      await page
      ...
      await page.getByText(/当前卡组：主卡.额外共 40 张/).waitFor();
      await page
        .locator("summary")
        .filter({ hasText: "房间信息与链接" })
        .click();
      await page.locator(".native-connection code").waitFor();
      assert.equal(
        await page.locator(".native-connection code").textContent(),
        new URL(page.url()).pathname.split("/").pop(),
      );
      await page
        .getByRole("button", { name: "复制密码", exact: true })
        .waitFor();
      await page
        .locator("summary")
        .filter({ hasText: "房间信息与链接" })
    ...
    for (const p of pages) {
      await p.waitForFunction(() =>
        Array.from(document.querySelectorAll("button")).some(
          (b) => /^(准备|取消准备)$/.test(b.textContent) && !b.disabled,
        ),
      );
      if (
        await p.getByRole("button", { name: "准备", exact: true }).isVisible()
      ) {
    ...
    if (process.env.EFFECT_TEST) {
      await active
        .locator(".duel-hand:not(.opponent-hand)")
        .getByRole("button", { name: "增援", exact: true })
        .first()
        .click();
      assert.equal(
        await active
          .locator(".duel-dialog")
          .getByRole("button", { name: "发动效果", exact: true })
          .count(),
        1,
      );
      await active
      ...
      await active
        .getByRole("button", { name: "墓地 1", exact: true })
        .waitFor();
      console.log(
        "PASS real host effect activation, search response during animation, chain resolution",
      );
    } else {
      await active
        .locator(".duel-hand:not(.opponent-hand) .card-selectable")
        .first()
        .click();
      await active
        .locator(".duel-dialog")
        .getByRole("button", { name: "通常召唤", exact: true })
        .click({ button: "right" });
      assert.equal(
        await active.locator(".duel-operation-modal").count(),
        0,
        "right click closes the operation menu",
      );
      await active
        .locator(".duel-hand:not(.opponent-hand) .card-selectable")
        .first()
      ...
      await active
        ...
        .getByRole("button", { name: "通常召唤", exact: true })
        .click();
      await active.locator(".duel-zone-target").first().waitFor();
      const zonesBefore = await active.locator(".duel-zone-target").count();
      await active
        .locator(".duel-zone-target")
        .first()
        .click({ button: "right" });
      assert.equal(
        await active.locator(".duel-zone-target").count(),
        zonesBefore,
        "right click cannot bypass a required field slot",
      );
      await active.locator(".duel-zone-target").first().click();
      await active
        .locator(".duel-half:not(.opponent) .duel-zone-row")
        .first()
        .locator(".duel-card")
        .waitFor();
    ...
    await builder
      .locator(".deck-inspector h2")
      .filter({ hasText: "青眼" })
      .waitFor();
    await builder.screenshot({
      path: require("node:path").join(
        require("node:os").tmpdir(),
        "standalone-builder-390.png",
      ),
      fullPage: true,
    });
    assert(
    ...
    await builder
      .getByRole("button", { name: "绑定本地图像目录", exact: true })
      .waitFor();
    const roomUrl = a.url(),
      watchContext = await browser.newContext({
        viewport: { width: 390, height: 900 },
      }),
      watch = await watchContext.newPage();
    await watch.goto(roomUrl);
    ...
    assert.equal(
      await watch.getByRole("button", { name: "投降", exact: true }).count(),
      0,
    );
    await contexts[0].close();
    if (process.env.RECONNECT_DELAY_MS) {
      console.log("Waiting beyond the former reconnect window");
      await new Promise((resolve) =>
        setTimeout(resolve, Number(process.env.RECONNECT_DELAY_MS)),
      );
    }
    const reconnectContext = await browser.newContext({
        viewport: { width: 390, height: 900 },
      }),
      resumed = await reconnectContext.newPage();
    await resumed.goto(roomUrl);
    await resumed.getByLabel("玩家 ID", { exact: true }).fill("BrowserAlice");
    await resumed
      .getByRole("button", { name: "使用 ID 重连", exact: true })
      .click();
    await resumed.getByText("已连接", { exact: true }).waitFor();
    await resumed
      .locator(".duel-hand:not(.opponent-hand) button")
      .first()
      .waitFor();
    await b.getByRole("button", { name: "投降", exact: true }).click();
    await b.getByRole("button", { name: "确认", exact: true }).click();
    await resumed
      .locator(".duel-outcome")
      .filter({ hasText: "投降" })
      .waitFor();
