...
(async () => {
 ...
 try {
  for (const width of (process.env.DUEL_WIDTHS ? JSON.parse(process.env.DUEL_WIDTHS) : [390,1440])) for (const seat of [0,1]) {
   ...
   await page.screenshot({path:`/tmp/duel-zones-${width}-${seat}.png`,fullPage:true});
   ...
   await page.screenshot({path:`/tmp/duel-effects-${width}-${seat}.png`,fullPage:true});
   ...
   assert.equal(await page.locator('[data-deck-area="extra"] .duel-side-grid button').count(),1);
   await page.screenshot({path:`/tmp/duel-siding-${width}-${seat}.png`,fullPage:true});
   ...
   for(const height of width<700?[667,900]:[600,1000]){
    ...
    const board=await page.locator('.duel-table').boundingBox();if(!(board.y>=0 && board.y+board.height<=height)){await page.screenshot({path:'/tmp/duel-fit-failure.png',fullPage:true});console.log(await page.locator('.duel-table > *').evaluateAll(es=>es.map(e=>[e.className,e.getBoundingClientRect().height])));console.log({board,height});}assert(board.y>=0 && board.y+board.height<=height,'whole field fits viewport');
    await page.screenshot({path:`/tmp/duel-uniform-${width}-${height}-${seat}.png`,fullPage:true});
   }
