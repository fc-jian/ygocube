...
(async()=>{
 ...
 try {for(const aliasKind of ['rule','artwork']) for(const width of [1440,390]){
  cards[0].aliasKind=aliasKind; cards[0].code=aliasKind==='artwork'?223:111;
  ...
  for(const value of ['灵魂','测试字段',`${aliasKind==='artwork'?'异画版本':'规则同名'}：规则原名`,'完整效果第二行']) assert(text.includes(value),value);
  ...
  await page.close();console.log(JSON.stringify({width,aliasKind,details:true,alias:true,banlists:true,overflow:false}));
