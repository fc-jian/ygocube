...
(async()=>{
 ...
 try {
  ...
  await page.getByRole('button',{name:'关闭提示 / Dismiss',exact:true}).click();assert.equal(await page.getByRole('alert').count(),0);
  await lock.click();await page.getByRole('alert').waitFor();
  await page.getByRole('alert').waitFor({state:'detached',timeout:10000});
  await lock.click();await page.getByRole('alert').waitFor();failLock=false;await lock.click();
  await page.getByRole('alert').waitFor({state:'detached'});
  failLoad=true;await page.reload();await page.getByRole('button',{name:'重新加载',exact:true}).waitFor();
  assert((await page.getByRole('alert').innerText()).includes('比赛状态加载失败'));
